      ...
      .then(json => {
		   this.setState({latitudepribadi:json.data.result[0].latitudepribadi});
			...
			this.setState({nama:json.data.result[0].nama});
				this.setState({jenis:json.data.result[0].jenis});
	 ...
	 var email =	this.props.route.params.email; 
      if(this.state.nama == 'sadas'){
        alert('Silakan masukkan nama dan alamat');
          ...
          .then((response)=>response.json())
          .then((json)=>{
              alert ('Bantuan Telah Selesai di Kerjakan')
...
alert ('Tidak Ada Yang Sedang Memerlukan Bantuan Panic Button')
      this.props.navigation.replace('HomeTravel',{email:email,name:name, age:age});
		...
		var age =	this.props.route.params.age; 
	  		 var latitudepribadi=  + this.state.latitudepribadi;
      var longitudepribadi =  + this.state.longitudepribadi;
      ...
      <ScrollView>
        <View
          ...
          <View style={{marginTop: 20 }}>
            <Text style={{fontSize: 16, fontWeight: 'bold' ,color:'gray'}}>Ketua Kloter</Text>
            <Text style={{fontSize: 16, fontWeight: 'bold',color:'red'}}>     <View>
            {
                this.state.listData.map((val,index)=>(
                   
                        <Text style={{marginLeft: -20 ,fontSize: 18, fontWeight: 'bold',color:'red'
                    }}>{val.nama_g}</Text>
		    ...
		    <View style={{marginTop: 20 }}>
            <Text style={{fontSize: 16, fontWeight: 'bold' ,color:'gray'}}>Telepon</Text>
            <Text style={{fontSize: 16, fontWeight: 'bold',color:'red'}}>     <View>
            {
                this.state.listData.map((val,index)=>(
		      ...
		      <View style={{marginTop: 20 }}>
            <Text style={{fontSize: 16, fontWeight: 'bold' ,color:'gray'}}>Embarkasi</Text>
            <Text style={{fontSize: 16, fontWeight: 'bold',color:'red'}}>     <View>
            {
                this.state.listData.map((val,index)=>(
...
<Marker
   coordinate={{
   latitude: + this.state.latitudepribadi,
      longitude:  + this.state.longitudepribadi,
   }}
   key ={'userpelanggan'}
